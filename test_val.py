...
class Detector(object):
    ...
    def calc_output(self, output):
        ...
        boxes = np.reshape(output[:, :, :, :4],
                           [self.cell_size, self.cell_size, self.box_per_cell, 4])  # boxes coordinate
        boxes = self.get_boxes(boxes) * self.image_size

        confidence = np.reshape(output[:, :, :, 4], [self.cell_size, self.cell_size,
                                                     self.box_per_cell])  # the confidence of the each anchor boxes
        confidence = 1.0 / (1.0 + np.exp(-1.0 * confidence))
        confidence = np.tile(np.expand_dims(confidence, 3), (1, 1, 1, self.num_classes))

        classes = np.reshape(output[:, :, :, 5:],
                             [self.cell_size, self.cell_size, self.box_per_cell, self.num_classes])  # classes
        classes = np.exp(classes) / np.tile(np.expand_dims(np.sum(np.exp(classes), axis=3), axis=3),
        ...
        for i in range(len(probs_filter)):
            ...
            for j in range(i + 1, len(probs_filter)):
                if self.calc_iou(box_filter[i], box_filter[j]) > 0.2:  # 修改了iou阈值
                    probs_filter[j] = 0.0
    ...
    def get_boxes(self, boxes):
        ...
        boxes1 = np.stack([(1.0 / (1.0 + np.exp(-1.0 * boxes[:, :, :, 0])) + offset) / self.cell_size,
                           (1.0 / (1.0 + np.exp(-1.0 * boxes[:, :, :, 1])) + np.transpose(offset,
                                                                                          (1, 0, 2))) / self.cell_size,
    ...
    def calc_iou(self, box1, box2):
        width = min(box1[0] + 0.5 * box1[2], box2[0] + 0.5 * box2[2]) - max(box1[0] - 0.5 * box1[2],
                                                                            box2[0] - 0.5 * box2[2])
        height = min(box1[1] + 0.5 * box1[3], box2[1] + 0.5 * box2[3]) - max(box1[1] - 0.5 * box1[3],
                                                                             box2[1] - 0.5 * box2[3])
    ...
    def draw(self, image, result):
        ...
        for i in range(len(result)):
            ...
            cv2.putText(image, result[i][0] + ':%.2f' % result[i][5], (xmin + 1, ymin + 8),
...
def main():
    ...
    parser.add_argument('--weights', default='yolo_weight.ckpt-1250', type=str)  # darknet-19.ckpt
